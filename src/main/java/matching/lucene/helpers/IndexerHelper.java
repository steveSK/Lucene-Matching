...
public class IndexerHelper {
    ...
    public IndexerHelper(String indexDirectoryPath,String spellCheckerDirPath, Analyzer analyzer) throws IOException {
        ...
        this.spellCheckerDir = FSDirectory.open(new File(spellCheckerDirPath).toPath());
    }


    public void makeSpellCheckerIndex(String field) throws IOException {
        IndexReader wcDirectory = DirectoryReader.open(indexDirectory);
        LuceneDictionary dict = new LuceneDictionary(wcDirectory,field);
        SpellChecker spellchecker = new SpellChecker(spellCheckerDir);
        spellchecker.indexDictionary(dict, new IndexWriterConfig(new KeywordAnalyzer()), false);
        wcDirectory.close();
    ...
    public void indexCSVFile(File file, LuceneSchema schema) throws IOException {
        ...
        IndexWriterConfig config = new IndexWriterConfig(getPerFieldAnalyzerWrapper(schema));
        IndexWriter writer = new IndexWriter(indexDirectory,config);
    ...
    public void clearIndex() throws IOException {
        ...
        writer.deleteAll();
        writer.close();
        IndexWriterConfig config2 = new IndexWriterConfig(new KeywordAnalyzer());
